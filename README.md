...
- **Events.** All categories (Open/Intermediate/Masters, singles/doubles/mixed) are pre-created. Each has its own groups, formats, and bracket. Events are started and paused from Admin, so a 4-day schedule is just: start today's event(s), pause or finish them, start tomorrow's. An event with a planned start time turns itself on 5 minutes early, and will do so even while another event is still playing, since the courts are shared anyway. It will not do so if the event already has a played match, so an event paused overnight is resumed by hand rather than restarting itself on day two off day one's start time. A manual Start or Pause always wins; saving a new time re-arms it. Schedule notes (e.g. "Sat 9am") show players what runs when. An event can be hidden from players, which is reversible and keeps everything, or deleted outright after typing its name to confirm. Hiding is the one to reach for mid-tournament.
- **Participants.** The desk's roster of people, on its own tab that only appears once Admin is unlocked. One card per person: name, gender, a singles and a doubles DUPR rating, and their DUPR ID, entered once and reused. Tick the events each person is entered in and name their partner; both halves of a pair have to tick the same event and name each other, and anyone left unmatched is listed by name rather than quietly guessed at. An event that has "Take entries from the Participants tab" ticked then gets a "Fill the rows from Participants" button in its draw block, which loads the complete pairs (with their doubles ratings and DUPR IDs) into the entry grid for a last look before Draw groups. Unlike everything else here the roster is not public: the table has no read policy and no grants, so gender and DUPR IDs are only ever visible to an unlocked desk.
...
- **Formats.** Per event and per stage: games to N points, best of 1, 3, or 5. Best-of matches record each game's score. New events default to group games best of 1 to 21 (15 for singles) and knockouts best of 3 to 11. Knockout games always play win by 2; the "Win by 2 in the group stage" tick is off by default, so a group game ends dead on the target even at 20-21, which keeps the group stage a predictable length on shared courts.
...
- No-show or a late team: use a waiting row's "Move to…" picker to send it further down, or all the way to the back. Players not ready on court: open the match, "Take off court", and either bring someone else on or hold it until they turn up. Wrong score: tap the match anywhere, fix or clear it.
...
- Fresh rosters: enter people once on the Participants tab and tick the events they are in, then press "Fill the rows from Participants" in that event's draw block. Otherwise, one row per entrant in the Admin entry boxes (name, DUPR, DUPR ID per player), or fill the rows from a CSV. With a header row the columns are matched by name, which is the only way to tell a DUPR ID from a rating.
